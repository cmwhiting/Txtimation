<!DOCTYPE html>
<html>
	<head>
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<title>Chad</title> 
		<script src="http://platform.twitter.com/anywhere.js?id=nBYpqwVnoGycZzzFXnmdw&v=1" type="text/javascript"></script>
		<script src="jquery-1.7.2.js" type="text/javascript"></script>
		<script type="text/javascript" src="main.js"></script>

		<link href="main.css" rel="stylesheet" />
	</head>

	<body>
		<header data-id="header" data-position="absolute">
			<nav>
				<a href="javascript:void(0)" class="menu">
					<span class="nav-img"></span>
					Menu
				</a>
				<div class="menu-sub">
					<a href="#">About Txtimation</a>
					<a href="#">About the Author</a>
					<a href="#">F.A.Q.</a>
					<a href="#">Help</a>
				</div>
				<a href="new.php" data-transition="slide" class="tweet menu-link">
					<span class="nav-img"></span>
					New
				</a>
				<a href="recent.php" data-transition="slide" class="recent menu-link">
					<span class="nav-img"></span>
					Recent
				</a>
				<a href="index.php" data-transition="slide" class="home menu-link">
					<span class="nav-img"></span>
					Home
				</a>
			</nav>
		</header>